...
async def test() -> None:
    ...
    with TimeFrame('test args', name='Text request', rt=func, test_kwargs='test kwargs') as time_frame:
        with time_frame.create_action(name='Prompt request', retries=5, ignore_retries=(TimeoutError,),
                                 check_exc_subclass=True) as event_frame:
            function_call = False
            for frame in event_frame:
                async with frame:
                    ...
                    if random.random() < 0.9:
                        function_call = True
        if function_call:
            with time_frame.create_action(name='Function call `mul`', retries=0):
                time.sleep(random.uniform(0, 0.01))  # Stimulate doing network operation
            with time_frame.create_action(name='Function Response', retries=5) as event_frame:
                for frame in event_frame:
                    async with frame:
                        time.sleep(random.uniform(0, 0.4))  # Stimulate doing network operation
